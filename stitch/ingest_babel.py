...
def _get_args() -> argparse.Namespace:
    ...
    arg_parser.add_argument('--temp-dir', dest='temp_dir', default=None,
                            help="specify an alternate temp directory;  "
                            f"the default is {tempfile.gettempdir()}")
...
_log_print_impl, _set_log_print_enabled = su.make_log_print_controller()

@contextlib.contextmanager
def _sqlite_transaction(conn: sqlite3.Connection) -> Iterator[None]:
    """Run the ``with`` body inside an explicit ``BEGIN``/``COMMIT``.

    Any exception inside the body rolls the transaction back and re-raises
    with the original traceback preserved.
    """
...
def _make_url_ingester(conn: sqlite3.Connection,
    ...
    def ingest_from_url(url: str,
        ...
        chunk_ctr = 0
        start_time = None
        for chunk in read_chunks(url, lines_per_chunk, temp_dir):
            chunk_ctr += 1
...
def _get_compendia_files(compend_files_index_url: str) ->\
        tuple[tuple[str, ...], dict[str, FileEntry]]:
    # ingest OrganismTaxon compendia file first
    compend_listing: list[FileEntry] = su.list_dir(compend_files_index_url)
...
def _get_conflation_files(conflation_files_index_url: str) ->\
        tuple[tuple[str, ...], dict[str, FileEntry]]:
    conflation_listing: list[FileEntry] = su.list_dir(conflation_files_index_url)
...
def _apply_pragmas(conn: sqlite3.Connection, pragmas: Iterable[str]) -> None:
    """Execute each ``PRAGMA <statement>`` in ``pragmas`` and log it."""
    for statement in pragmas:
        conn.execute(f"PRAGMA {statement}")
        _log_print(f"setting PRAGMA {statement}")
...
def _set_pragmas_for_querying(conn: sqlite3.Connection):
    _apply_pragmas(conn, ('wal_checkpoint(FULL)', 'journal_mode = DELETE'))
...
def _cleanup_indices(conn: sqlite3.Connection,
                     log_work: bool):
    _do_index_analyze(conn, log_work, True)
    _apply_pragmas(conn, ('locking_mode=EXCLUSIVE',))
    _run_vacuum(conn)
    _do_integrity_check(conn)
    # VACUUM is the only step that needs the exclusive lock; drop back to
    # NORMAL so the function leaves the connection in query-friendly state
    # (matching the intent of _set_pragmas_for_querying).
    _apply_pragmas(conn, ('locking_mode=NORMAL',))
...
def _customize_temp_dir(temp_dir: str,
    ...
    if not quiet:
        _log_print(f"Re-execing ingest_babel.py with requested temp dir: {temp_dir}")
    ...
    def ingest_urls(
        ...
        for file_name in file_names:
            ...
            if not dry_run:
                ...
                glbl_chnk_cnt = ingest_url(
                    urljoin(base_url, file_name), temp_dir, process_chunk,
                    total_size=file_size, glbl_chnk_cnt=glbl_chnk_cnt )
...
def _main_args(babel_compendia_url: str,
    ...
    if not quiet:
        _log_print(f"Temp path: {temp_dir_path}")
    ...
    with _get_database(database_file_name,
        ...
        do_ingest_compendia_url = \
            _make_url_ingester(conn, lines_per_chunk,
                               su.read_json_lines_from_url, log_work)
        do_ingest_conflation_url = \
            _make_url_ingester(conn, lines_per_chunk,
                               su.read_line_chunks_from_url, log_work)
        ...
        base_args = {"start_time_sec": start_time_sec,
                     "glbl_chnk_cnt": glbl_chnk_cnt,
                     "temp_dir": temp_dir_path}
        ingest_args_compendia = base_args.copy()
        ...
        ingest_args_conflation = base_args.copy()
        ...
        if test_type == 1:
            ...
            # Resolve the local test file to a file:// URL so the ingest path
            # routes through read_*_from_url's local-file branch (which reads
            # directly, no temp copy) instead of treating the bare filename as
            # an HTTP URL.
            test_path = Path(test_compendia_file).resolve()
            ingest_args_compendia.update({
                "file_names": (test_path.name,),
                "base_url": test_path.parent.as_uri().rstrip("/") + "/",
                "file_map": {test_path.name: FileEntry(
                    test_path.name,
